...
if not all([openai.api_key, STABILITY_API_KEY, GOOGLE_CREDENTIALS_PATH, GOOGLE_PROJECT_ID, FAL_KEY]):
    missing_keys = [];
    if not openai.api_key: missing_keys.append("OPENAI_API_KEY")
    if not STABILITY_API_KEY: missing_keys.append("STABILITY_API_KEY")
    if not GOOGLE_CREDENTIALS_PATH: missing_keys.append("GOOGLE_APPLICATION_CREDENTIALS")
    if not GOOGLE_PROJECT_ID: missing_keys.append("GOOGLE_CLOUD_PROJECT")
    if not FAL_KEY: missing_keys.append("FAL_KEY")
    st.error(f"Folgende Umgebungsvariablen fehlen in Ihrer `.env`-Datei: {', '.join(missing_keys)}")
    st.stop()
...
def testbed_page():
    ...
    if st.session_state[key("results")]:
        ...
        cols = st.columns(len(sorted_results) or 1)
